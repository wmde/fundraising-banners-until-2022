...
* Move translatable strings from `banner_functions.js`, `campaign_day_sentence.js` and `custom_day_name.js` into a central `messages.js` file and add a `Translations` object that can receive keys (and placeholder values) and returns translated strings. 
