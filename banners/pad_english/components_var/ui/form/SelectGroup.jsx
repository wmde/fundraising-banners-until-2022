...
export function SelectGroup( props ) {
	...
	return <div
		...
		<div className="select-group">
			{ props.selectionItems.map( ( { value, label, icon, notice } ) => {
				const noticeLabel = notice ? ( <span className="select-group__notice">{ notice }</span> ) : '';
				return (
					<label key={ value } className={ classNames(
						...
						<span className="select-group__state">{ icon && <span className={ icon }></span> }{ label || value }</span>
						{noticeLabel}
					</label> );
			} )
			}
